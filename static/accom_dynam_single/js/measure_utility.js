...
//variables used for computing new utilities while dragging
  ...
  function dragged() {
    if(yScale.invert(d3.event.y) > 0){
        newY = Math.min(Math.max(yScale.invert(d3.event.y),0.0001), max_value);
    } else {
        newY = Math.max(Math.min(yScale.invert(d3.event.y),-0.0001), -1*max_value);
    };
    ...
    if (singleDrag) {
        ...
        otherUtilities[i] = newY;
        other[i][1] = otherUtilities[i];
